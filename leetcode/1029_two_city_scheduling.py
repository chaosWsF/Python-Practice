"""
A company is planning to interview 2n people. Given the array costs where costs[i] = [aCost_i, bCost_i], the 
...
class Solution:
    def twoCitySchedCost1(self, costs) -> int:
        ...
        for a, b in costs:
            res += a
            costs_diff.append(b-a)

        costs_diff.sort()
        return res + sum(costs_diff[:len(costs)//2])

    def twoCitySchedCost2(self, costs) -> int:
        n = len(costs) // 2
        costs.sort(key=lambda x: x[1] - x[0])
        return sum(x[1] for x in costs[:n]) + sum(x[0] for x in costs[n:])
