...
its moves, judge if this robot ends up at (0, 0) after it completes its moves. The move sequence 
is represented by a string, and the character moves[i] represents its ith move. Valid moves are 
R (right), L (left), U (up), and D (down). If the robot returns to the origin after it finishes 
...
class Solution:
    def judgeCircle(self, moves):    # 40ms
        moves = Counter(moves + 'UDRL')
        return moves['U'] == moves['D'] and moves['R'] == moves['L']

    def judgeCircle2(self, moves):    # 32ms
